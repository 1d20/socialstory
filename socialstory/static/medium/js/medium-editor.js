    ...
    MediumEditor.prototype = {
        ...
        bindParagraphCreation: function(index) {
            ...
            self.elements[index].addEventListener('keyup', function(e) {
                ...
                // Only if there is shift is not pressed
        ...
        setToolbarPosition: function() {
            ...
            var buttonHeight = 50,
                ...
                defaultLeft = (self.options.diffLeft) - (self.toolbar.offsetWidth / 2),
                middleBoundary = (boundary.left + boundary.right) / 2,
                halfOffsetWidth = self.toolbar.offsetWidth / 2;
            if (boundary.top < buttonHeight) {
                self.toolbar.classList.add('medium-toolbar-arrow-over');
                self.toolbar.classList.remove('medium-toolbar-arrow-under');
                self.toolbar.style.top = buttonHeight + boundary.bottom - self.options.diffTop + window.pageYOffset - self.toolbar.offsetHeight + 'px';
            } else {
                self.toolbar.classList.add('medium-toolbar-arrow-under');
                self.toolbar.classList.remove('medium-toolbar-arrow-over');
                self.toolbar.style.top = boundary.top + self.options.diffTop + window.pageYOffset - self.toolbar.offsetHeight + 'px';
            }
            ...
            // /* -> My own fix */
            // if (!selection.anchorNode.parentElement.getAttribute('data-element-id')){
            //     selection.anchorNode.parentElement.setAttribute('data-element-id', (++self.paragraphsCount));
            //     console.log(self.paragraphsCount);
            // }
            // /* My own fix <- */
            return this;
        },
        ...
        execFormatBlock: function(el) {
            ...
            if (elementId === undefined) {
                elementId = ++self.paragraphsCount;
            };
            // Only for FF
            ...
            this.selection.anchorNode.parentElement.dataset.elementId = elementId;
            return;
        },
                    ...
                    if (e.clipboardData && e.clipboardData.getData) {
                        ...
                        if (!self.options.disableReturn) {
                            ...
                            for (p = 0; p < paragraphs.length; p += 1) {
                                if (paragraphs[p] !== '') {
                                    html += '<p data-element-id="' + (++self.paragraphsCount) + '">' + paragraphs[p] + '</p>';
                                }
                            }
